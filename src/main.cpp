...
// Copyright (c) 2014 The Entropycoin developers
...
uint256 hashGenesisBlock("0x8b7020a5f83d7c90ca2e6a1685a5c21db5afff52916539ccf06feb38e96d0552"); 


static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Entropycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Entropycoin Signed Message:\n";
...
{
    // Entropycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)
	{
		nSubsidy = 1450000 * COIN; // 1.45 mil < 1%
		return nSubsidy + nFees;
	} 
	
	//Add Rewards For Early Adopters
	int dayNum = nHeight / 1440 + 1;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Entropycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Entropycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 20/Feb/2014 Ukraine Crisis Deepens as Death Toll Climbs"; //http://www.nytimes.com/2014/02/21/world/europe/ukraine.html?hp
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1392945252;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2070339;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Entropycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EntropycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EntropycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EntropycoinMiner(CWallet *pwallet)
{
    printf("EntropycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("entropycoin-miner");
    ...
    try { loop {
        ...
        printf("Running EntropycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EntropycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EntropycoinMiner, pwallet));
}
